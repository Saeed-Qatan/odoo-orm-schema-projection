-- EXPANDED TEST FIXTURE, NOT AN AUTHENTIC ODOO SALES EXPORT.
-- Base: repository schema.full.sql, preserved unchanged below.
-- Appended sales/product definitions are synthetic metadata for parser,
-- ORM mapping and projection scale tests. No business data is included.
-- Do not execute this file against an operational database.
--
...
-- BEGIN SYNTHETIC SALES/PRODUCT EXTENSION
CREATE TABLE public.product_category (
    id integer PRIMARY KEY,
    name character varying NOT NULL,
    parent_id integer REFERENCES public.product_category(id)
);
...
CREATE TABLE public.product_template (
    ...
    name character varying NOT NULL,
    list_price numeric,
    categ_id integer REFERENCES public.product_category(id),
    active boolean NOT NULL DEFAULT true
);

CREATE TABLE public.product_product (
    id integer PRIMARY KEY,
    product_tmpl_id integer NOT NULL REFERENCES public.product_template(id),
...
CREATE TABLE public.sale_order (
    id integer PRIMARY KEY,
    name character varying NOT NULL,
    partner_id integer NOT NULL REFERENCES public.res_partner(id),
    user_id integer REFERENCES public.res_users(id),
    date_order timestamp without time zone,
    amount_total numeric,
    state character varying,
    company_id integer NOT NULL REFERENCES public.res_company(id),
    currency_id integer NOT NULL REFERENCES public.res_currency(id)
);

CREATE TABLE public.sale_order_line (
    id integer PRIMARY KEY,
    order_id integer NOT NULL REFERENCES public.sale_order(id),
    product_id integer REFERENCES public.product_product(id),
    product_uom_qty numeric,
    price_unit numeric,
    discount numeric,
    price_subtotal numeric
);
-- END SYNTHETIC SALES/PRODUCT EXTENSION
