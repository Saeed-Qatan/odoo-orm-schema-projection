...
def create_projection_router(pipeline: SchemaProjectionPipeline, limiter: InMemoryRateLimiter, settings: Settings | None = None) -> APIRouter:
    router = APIRouter(prefix="/api/v1", tags=["projection"])

    def project_limit(request: Request) -> None:
        runtime_settings = settings or get_settings()
        limiter.check(request, runtime_settings.project_schema_rate_limit_per_minute, "project-schema")
    ...
    async def project_schema(payload: ProjectionRequest) -> ProjectionResponse:
        ...
        try:
            return await asyncio.wait_for(
                ...
                timeout=runtime_settings.request_timeout_ms / 1000,
            )
